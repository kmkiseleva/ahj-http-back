...
app.use(async (ctx) => {
  ...
  switch (method) {
    ...
    case "createTicket":
      ...
      return;
    case "deleteById":
      const deleteIndex = tickets.findIndex((ticket) => ticket.id === id);
      tickets.splice(deleteIndex, 1);
      ctx.response.body = tickets;
      return;
    case "updateById":
      const updateIndex = tickets.findIndex((ticket) => ticket.id === id);
      const updateData = JSON.parse(ctx.request.body);
      const ticket = {
        ...tickets[updateIndex],
        ...updateData,
      };
      tickets.splice(updateIndex, 1);
      tickets.splice(updateIndex, 0, ticket);
      ctx.response.body = tickets;
      return;
    default:
